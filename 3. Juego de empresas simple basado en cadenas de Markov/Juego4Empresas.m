clear all; %se vacían todas las variables
...
cargar = Unodos(); %Se comprobando que se ha seleccionado 1 ó 2
...
if cargar==1 %Si se carga una partida antigua
    fprintf('¿Qué nombre tiene el fichero de entrada?\n')
    ...
    if nombrec==1
        ...
        while F1==-1
            fprintf('No existe ninguna partida con ese nombre. Escriba el nombre de un fichero válido.\n')
            ficheroc=input('','s');
    ...
    [UD]=fscanf(F1,'\n %9d       %9d     %9d    %9d,  ');%Número entero con 9 caracteres
    %fscanf(F1,'%s',6);
    ...
    %fscanf(F1,'%s',5);
    [PVP]=fscanf(F1,'%10f          %10f     %10f    %10f,   ');%Número decimal con 10 caracteres
    pvpini=sum(PVP)/4;
    %fscanf(F1,'%s',4);
    ...
    fscanf(F1,'%s',1);
    if ruptadm==2 %el cliente no está dispuesto a esperar
    %    fscanf(F1,'%s',13);
        [Crupt]=fscanf(F1,'%f %f %f %f'); % Coste de Ruptura
    %    fscanf(F1,'%s',1);
    else          %el cliente sí está dispuesto a esperar
    %    fscanf(F1,'%s',13);
...
%% Datos obtenidos de la Partida Antigua
    ...
    fprintf('\n|_______________________|________________|______________|________________|_______________|')
    %  "%8.2f" 6 números enteros y 2 decimales
    ...
    if ruptadm==2 %el cliente no está dispuesto a esperar
        fprintf('\n (rupdtadm= 2)El cliente no está dispuesto a esperar con un coste de ruptura de %.2f %.2f %.2f %.2f',Crupt(1,1),Crupt(2,1),Crupt(3,1),Crupt(4,1));
    else          %el cliente sí está dispuesto a esperar
        fprintf('\n (rupdtadm= 1)El cliente está dispuesto a esperar con un coste de no servicio de %.2f %.2f %.2f %.2f',ruptadm(1,1),Cnoserv(2,1),Cnoserv(3,1),Cnoserv(4,1));
    end
    ...
    while duracion<=0 || entero~=0
        fprintf('La duración elegida no es válida. Escriba otro valor.\n')
        duracion=input('');
...
%% Partida Nueva
    ...
    %fprintf('\nTéngase en cuenta que este valor permanecerá constante durante toda la partida')
    ...
    fprintf('En caso de ruptura, ¿Se permite entregar el pedido en el siguiente período?\n')
    fprintf('1) Escriba 1 en caso afirmativo.\n')
    fprintf('2) Escriba 2 en caso contrario.\n')
    ruptadm= Unodos(); % Debe ser 1 ó 2.

    if ruptadm==1 %Si el cliente está dispuesto a esperar
        fprintf('\nEscriba el coste de no servicio de la empresa %s:\n', Empresa(i))
        cnoserv=Positiva(); %mayor que cero y puede ser no entero
        Cnoserv(i,1)=cnoserv;

    else  %ruptadm==2, por lo que el cliente no está dispuesto a esperar
...
%clc;
    ...
    fprintf('\n          Se dirá que variación de demanda mínima será -10 (valor expresado en porcentaje).');
    ...
    fprintf ('\n\n\n Incremento estimado de demanda máximo (en porcentaje):\n');
    dem_maxima=input(''); %Incremento máximo de demanda (en porcentaje)
    ...
    while dem_maxima< -50 || dem_maxima > 200 || entero~=0 || dem_minima>dem_maxima
        fprintf('La demanda máxima del mercado esperada debe tomar un valor entero entre -50 y 200 e igual o superior a la demanda mínima\n')
        fprintf('El valor introducido no es correcto. Escriba otro porcentaje.\n')
        dem_maxima=input('');
        entero=mod(dem_maxima,1);
    end

    %la demanda mínima y la máxima en ventas se obtiene redondeando
...
%% Introducción de Datos para el Comienzo del Juego
        ...
        fprintf('\n Durante el pasado mes esta empresa %s vendió %d de las %d que se vendieron en total.', Empresa(i), V(i,j-1), ventasmax);
        ...
        if ruptadm==2 %El cliente no está dispuesto a esperar
            fprintf('\n El clientes no está dispuesto a esperar con un coste ruptura de %.2f',Crupt(i,1))
        else          %El cliente sí está dispuesto a esperar
            fprintf('\n El cliente está dispuesto a esperar con un coste de no servicio de %.2f',Cnoserv(i,1))
        end

        fprintf('\n Teniendo en cuenta las estrategias que se llevarán a cabo durante este mes,\n');
        fprintf('¿cuántas unidades desea poner a la venta? \n');
        ...
        while modA~=0  %Se muestra por pantalla las posibles estrategias a realizar
            ...
            fprintf('\n3) Escriba 3 si desea invertir en Mejoras Tecnológicas');
            fprintf('\n4) Escriba 4 si desea modificar las Unidades que se pretenden vender');
            fprintf('\n\n -------------------- Escriba 0 si no desea hacer nada --------------------\n');


            modA=input('');%Se comprueba si la elección es una entrada válida
            ...
            switch modA % Determina qué variable se va a comprobar (valores introducidos entre 0 y 4 de modA)
                ...
                case 3 %Si se elige 3 (invertir en Mejoras Tecnológicas)
                    [CTOTAL,CV,CM,mejoras]=Mejoras(i,j,CV,CTOTAL,PRESUPUESTO,Empresa,CM,mejoras,duracion);

                case 4 %Si se elige 4 (modificar Número de Unidades)
                    fprintf('\n\n ¿Cuantas unidades se pondrán a la venta este mes? \n');
                    UD(i,j)=Enteropos(); %Se comprueba que sea un número entero positivo 
            end
        end
        ...
        if isempty(a)==0 % Matriz no nula, por lo que existen valores negativos. Si no hubiera valores negativos, la matriz sería nula.
        ...
        %Después de obtener las nuevas ventas partiendo de la empresa que
        %se está analizando, se modifica la cadena de Markov de forma "ficticia" 
        %y sólo teniendo en cuenta las estrategias propias, no las del resto.
        %CM(:,i)=V(:,j)/sum(V(:,j));
    ...
    aleatorio=rand(1); % 0 --> demanda mínima; 1 --> demanda máxima
    porc_suerte= dem_minima + (dem_maxima-dem_minima)*aleatorio; % Porcentaje aleatorio de la demanda entre mínima y máxima.
    ventasmax=round(ventasmax*(100+porc_suerte)/100); %Ventas máximas del mercado redondeadas.
    [EVect,EVal]=eig(CM);
    cont1=1;
    while (1-EVal(cont1,cont1))^2>1e-10 %Búsqueda del autovalor unitario (con cierta tolerancia)
        cont1=cont1+1;
    end
    ...
    %V(:,j)=fix(CM*ventasmedia); %Teóricas ventas reales (redondeadas)
    V(:,j)=fix(ventasmedia); %Teóricas ventas reales (redondeadas)
    ...
    % realizado el mes anterior y que están pendientes de los clientes que
    ...
    for i=1:4 %Bucle en empresas
        if UD(i,j)< Ventasreales(i,j) %Si se han fabricado menos unidades de las que se deben vender
            ...
            while STOCK(i,j)>0 && faltan(i,j)>0 % Se vende de lo que queda en STOCK hasta que ya no queden
                %más existencias o unidades que vender.
                STOCK(i,j)=STOCK(i,j)-1;
                faltan(i,j)=faltan(i,j)-1;
            end


            if faltan(i,j)>0 %Si incluso así no se atiende a toda la demanda
                %se comprueba si el cliente está dispuesto a esperar
                Ventasreales(i,j)=Ventasreales(i,j)-faltan(i,j);

                if ruptadm==2 %Si el cliente no está dispuesto a esperar
                    CRUPT(i,j)=faltan(i,j)*Crupt(i,1); %Se tendrá un coste de ruptura
                else %Si el cliente está dispuesto a esperar
        ...
        elseif UD(i,j)> Ventasreales(i,j) %Si se han fabricado más unidades de las vendidas
...
%% Tabla Resumen de Cuentas
    ...
    if ruptadm==2 %el cliente no está dispuesto a esperar
        ...
        fprintf(F2,'\n (rupdtadm= 1)El cliente está dispuesto a esperar con un coste de no servicio de %.2f %.2f %.2f %.2f',Cnoserv(1,1),Cnoserv(2,1),Cnoserv(3,1),Cnoserv(4,1));
    end
...
if guardar==1
    fprintf('¿Qué nombre desea darle al fichero?\n')
